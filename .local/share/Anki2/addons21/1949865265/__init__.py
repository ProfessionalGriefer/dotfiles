...
def get_learning_table_html(data):
	...
	for ivl in sorted({i[1] for i in data}, reverse=True):
		m, s = divmod(abs(ivl), 60)
		columns = f"<td>{m} min{f' {s} s' if s else ''}</td>"
		for thetype in types:
			ivl_data = data.get((thetype, ivl))
			if ivl_data:
...
def get_data(self, max_ivl):
	...
	if isinstance(max_ivl, int) and max_ivl > 0:
		conds.append(f"lastIvl <= {max_ivl}")
...
def retention_graphs(self):
	...
	data = get_data(self, max_ivl)
	for ivl, thetype, correct, total in data:
		if thetype == 3:
			...
			learning_data[(thetype, ivl)] = (correct, total)
